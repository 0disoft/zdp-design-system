...
제품 헤더와 공용 앱 셸은 simple ship mark 단독 표시가 기본이다. `8ailors` 워드마크 lockup은 회사 소개, 캠페인, 넓은 editorial 표면에서만 선택적으로 쓴다.
...
공용 레몬 심볼과 Dinghy부터 Flagship까지의 크레딧 팩 함선은 브랜드 fallback과 분리된 package surface로 제공한다. 디자인 시스템은 형태와 렌더링 규칙만 소유하며 가격, 지급량, 보너스, 유효기간과 판매 상태를 결정하지 않는다.

```ts
import { zdpCreditAssets } from 'zdp-design-system/credit-assets';
import lemon from 'zdp-design-system/assets/credits/credit-lemon-simple-mono.svg';
...
24px compact glyph는 표·영수증·지갑 목록에 사용하고, 1600×900 cinematic key art는 선택 카드와 큰 요금제 상세 패널에 사용한다. 구매·보너스·무료·보상 레몬을 색상만으로 구분하지 않으며, 함선은 localized pack name과 가격·지급 정보를 대체하지 않는다. variant, 최소 크기, `currentColor`, 접근성 및 소유권 규칙은 [`docs/CREDIT_ASSETS.md`](docs/CREDIT_ASSETS.md)를 따른다.
...
- ResizableSplitPane은 데스크톱의 두 영역과 접근 가능한 경계선만 제공한다. 기본 폭은 280px, 기본 제한은 220–480px이며 drag, Arrow, Home/End, PageUp/PageDown, Shift 가속, 단일 click step, double-click reset을 지원한다. Astro와 정적 HTML은 같은 동작을 제공하는 framework-neutral `createZdpSplitPaneController`를 연결한다. 실제 저장 key와 좁은 화면의 Sheet 열림 상태는 소비 앱이 정한다.
- Tooltip은 짧은 보조 설명만 맡고, light DOM과 stylesheet가 설치된 open shadow root에서 Escape dismiss와 trigger focus를 유지하며, 긴 안내나 상태 설명은 Popover, Disclosure, 문서 본문으로 보낸다.
...
- Toast와 StatusToast는 저장, 동기화, 연결 같은 짧은 상태 피드백을 제공한다. StatusToast의 기본 ID는 SSR과 hydration에서 안정적이며, 명시적인 `idPrefix`도 계속 지원한다.
...
- TermTrigger와 TermSheet는 용어 설명을 click-open sheet로 연결하며, TermTrigger는 sheet가 열린 동안에만 `aria-controls`를 연결한다. TermSheet는 짧은 설명, 긴 설명, 예시, 관련 용어만 보여주며 detail page 링크나 라우팅 action을 제공하지 않는다. `open` 요청과 non-null `term`이 함께 있을 때만 modal lifecycle을 활성화하므로 비동기 term이 나중에 도착하거나 열린 상태에서 사라져도 scroll lock, inert, focus 복귀가 실제 DOM과 일치한다. 기본 dialog/title/description ID는 SSR과 hydration에서 안정적이며, 명시적인 `id`도 계속 지원한다. TermTrigger는 본문 안 의미 있는 단어이므로 hover에서 글자색을 바꾸지 않고 배경만 살짝 강조하며, 좌우 최소 padding과 focus token을 유지하고, 텍스트 선택을 막지 않는다. TermSheet root에는 stable `term_id`와 `data-zdp-ad-exclude`를 남긴다. TermSheet에는 광고 slot을 넣지 않는다.
- Dialog는 모달 레이어이며 닫기, scroll lock, focus trap, `describedBy`에 id 배열, `errorMessageId`로 `aria-errormessage` 연결을 명확히 한다. Menu, Popover, Combobox, Tooltip, Dialog, Sheet, TermSheet 같은 dismissible surface는 활성 동안만 document-scoped dismiss stack에 등록되고, 문서당 공유 click/keydown listener가 활성 순서의 최상단 하나만 닫는다. Tooltip은 outside click 후보에서는 건너뛰되 Escape 순서에는 참여한다. Dialog, Sheet, TermSheet의 modal registry는 browser document lifecycle에만 등록되므로 반복되거나 실패한 SSR 렌더 사이에 layer 상태를 남기지 않고, 처음부터 열린 SSR 표면은 hydration 뒤 scroll lock과 background inert를 활성화한 다음 close 때 복구한다.
- Checkbox와 Switch는 native input에 invalid 상태를 노출하고, Radio는 native radio role 제약 때문에 wrapper invalid styling과 `describedBy` 연결을 유지한다. Radio group은 동일한 `selectedValue`를 모든 Radio에 bind해 부모와 DOM이 단일 선택값을 공유하며 개별 `checked` prop은 제공하지 않는다. Switch는 `describedBy` id 배열과 `errorMessageId`로 에러 설명을 연결한다.
- Page, Container, Section, PageHeader는 페이지 폭, 섹션 rhythm, 제목 구조를 정리한다. 페이지 최외곽 shell은 `Container gutter="page"` 또는 `.zdp-container--padding-page`를 사용해 max-width의 auto margin이 사라지는 폭에서도 양쪽 최소 24 CSS px를 유지한다. `padding="sm"`, `padding="md"`, `padding="lg"`는 중첩 Container에 계속 쓸 수 있으며, 페이지 shell에서 더 작은 값을 쓰는 경우는 소비 저장소의 명시적 architecture 예외로 남긴다.
- Flutter는 Svelte 컴포넌트를 직접 쓰지 않고 `tokens/zdp.tokens.json`과 필요한 platform adapter를 통해 시각 토큰만 소비한다.

아래 package-root 예시는 기존 소비처 호환과 전체 컴포넌트 조합 검증용이다. 새 Svelte 또는 Tauri(Svelte) 코드는 위의 `components/ComponentName` direct subpath와 `tokens.css`를 사용한다.
...
Astro는 `styles.css`를 전역으로 불러오고, 정적 split pane에는 Svelte island 대신 `zdp-design-system/split-pane`의 `createZdpSplitPaneController`를 연결할 수 있다. framework-neutral helper는 Svelte component root barrel과 분리되어 Vite dev dependency optimizer가 `.svelte` 파일을 일반 JavaScript로 해석하지 않는다. controller는 import 시점에 `window`나 DOM을 읽지 않으므로 SSR import가 안전하고, 실제 DOM element는 브라우저에서만 전달한다.
...
const root = document.querySelector<HTMLElement>('[data-document-split-pane]');
const primary = root?.querySelector<HTMLElement>('[data-zdp-split-pane-primary]');
const separator = root?.querySelector<HTMLElement>('[data-zdp-split-pane-separator]');
const secondary = root?.querySelector<HTMLElement>('[data-zdp-split-pane-secondary]');

if (root && primary && separator && secondary) {
  const persistence = createZdpSplitPaneSizePersistence({ key: 'document-contents' });
  const controller = createZdpSplitPaneController(
    { root, primary, separator, secondary },
    {
      size: persistence.load(),
      minSize: 220,
      maxSize: 480,
      secondaryMinSize: 480,
      ariaLabel: '목차 너비 조절',
      onResizeCommit: (size) => persistence.save(size)
    }
  );
...
`920px` 이하에서는 소비 앱이 controller를 생성하지 않거나 `destroy()`하고, split pane 대신 접힘 메뉴나 `Sheet placement="left"`를 렌더링한다. 공유 dock은 split pane의 secondary 안에 넣어 폭 계산에 섞지 말고 바깥 `auto` track에 둔다. Flutter는 `tokens/zdp.tokens.json`을 Dart theme adapter 입력으로 쓴다.
