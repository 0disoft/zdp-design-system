...
- Dialog는 모달 레이어이며 닫기, scroll lock, focus trap, `describedBy`에 id 배열, `errorMessageId`로 `aria-errormessage` 연결을 명확히 한다. Menu, Popover, Combobox 같은 dismissible surface는 열린 동안만 document-scoped dismiss stack에 등록되고, 문서당 공유 click/keydown listener가 활성 순서의 최상단 하나만 닫는다. Dialog, Sheet, TermSheet의 modal registry는 browser document lifecycle에만 등록되므로 반복되거나 실패한 SSR 렌더 사이에 layer 상태를 남기지 않고, 처음부터 열린 SSR 표면은 hydration 뒤 scroll lock과 background inert를 활성화한 다음 close 때 복구한다.
