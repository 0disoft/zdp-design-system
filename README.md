...
Astro는 `styles.css`를 전역으로 불러오고, 정적 split pane에는 Svelte island 대신 `createZdpSplitPaneController`를 연결할 수 있다. controller는 import 시점에 `window`나 DOM을 읽지 않으므로 SSR import가 안전하고, 실제 DOM element는 브라우저에서만 전달한다.
