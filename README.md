...
- Checkbox와 Switch는 native input에 invalid 상태를 노출하고, Radio는 native radio role 제약 때문에 wrapper invalid styling과 `describedBy` 연결을 유지한다. Radio group은 각 항목의 `checked` boolean을 따로 bind하지 않고 동일한 `selectedValue`를 모든 Radio에 bind해 부모와 DOM이 단일 선택값을 공유한다. 기존 `checked` prop은 호환용 deprecated 표면이다. Switch는 `describedBy` id 배열과 `errorMessageId`로 에러 설명을 연결한다.
