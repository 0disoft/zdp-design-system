...
- `preview:check` invokes `styles:parity:check`, which covers base layout properties (sizing, spacing, positioning, flex/grid alignment and overflow), every interaction-state selector, and every conditional selector/declaration under its original context. Repeated selectors compose in cascade order; structural ancestors may widen to the same shared class, but variant-specific layout scope stays intact. Shared utility-only selectors remain allowed.
