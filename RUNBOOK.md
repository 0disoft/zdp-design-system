...
Consumer-visible package changes and release-tooling changes add a validated Markdown fragment under `.changes/`. The generated `release/zdp-design-system` branch is disposable: `release-pr.yml` rebuilds it from the latest `main`, applies the highest requested patch, minor, or major bump, moves pending notes into a versioned `CHANGELOG.md` section, and removes the consumed fragments. Do not place hand-written commits on that branch.
For the pre-1.0 package, intentional public breaking changes use a minor fragment unless the release explicitly crosses the 1.0 boundary. Compatible fixes use patch and compatible public features use minor. Repository-only maintenance does not need a fragment unless it changes packaged files or release behavior.
Merging the generated release pull request prepares metadata only. It does not publish, tag, or grant npm authority. Because pull requests created with `GITHUB_TOKEN` do not recursively trigger ordinary workflow events, `release-pr.yml` explicitly dispatches `design-system.yml` against the generated branch head.
Release preparation installs no project dependencies. A read-only job creates an integrity manifest, the write-authorized job independently reruns the built-in-only preparation script and requires byte-for-byte agreement, and a separate actions-only job dispatches generated-branch CI. Repository Actions settings must permit `GITHUB_TOKEN` to create pull requests, but generated pull requests still require human review and cannot approve or merge themselves.
...
The main package CI follows the same immutable Action policy, grants only `contents: read`, and disables checkout credential persistence.
