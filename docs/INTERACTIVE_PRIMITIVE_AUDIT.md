...
| Menu | ZDP custom menu | High | trigger keyboard open, roving focus, Home/End, Escape, outside click, focus return | typeahead, submenu, pointerdown outside, collision/portal 요구 시 Bits UI 후보 |
| Popover | ZDP custom non-modal overlay | High | Escape, outside click, focus return, role/dialog passthrough | first focus policy, collision/portal, nested overlay 요구 시 Bits UI 후보 |
