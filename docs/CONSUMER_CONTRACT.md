...
데스크톱 작업 영역은 `ResizableSplitPane`, `.zdp-resizable-split-pane`, `.zdp-resizable-split-pane__separator`로 나누되 현재 폭과 저장 key, 모바일 전환점, Sheet 열림 상태는 소비처가 소유한다.
...
- ResizableSplitPane을 쓰는 화면은 px `size`와 고유 persistence key를 소비 앱이 소유하고, separator의 이름·현재값·Arrow/Home/End 동작, 24px 이상 hit area, pointer cancel cleanup, RTL, 200% zoom, 긴 말줄임을 확인한다. 좁은 부모에서는 split pane을 계속 찌그러뜨리지 말고 Sheet나 접힘 메뉴로 바꾼다.
