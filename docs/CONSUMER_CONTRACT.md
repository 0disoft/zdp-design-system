...
- standalone consumer, public template, external example처럼 sibling checkout을 전제로 하지 않는 표면은 npm registry package를 사용한다. 0.50.3 이상에서는 `zdp-design-system: ^0.50.3`를 기본 semver 범위로 쓰고, 재현 가능한 release proof가 필요한 곳만 exact version을 pin한다.
...
`createZdpSplitPaneController({ root, primary, separator, secondary }, options)`는 같은 Pointer Events, keyboard, RTL, ResizeObserver clamp, class, CSS variable, `role="separator"`, `aria-orientation`, `aria-valuemin/max/now/text` 계약을 일반 DOM에 연결한다. Astro와 정적 HTML은 package root가 아니라 `zdp-design-system/split-pane`에서 import해 Svelte component barrel을 Vite dev dependency graph에 넣지 않는다. import 자체는 SSR-safe이며 DOM 조회와 controller 생성은 브라우저 lifecycle에서만 수행한다. `update()`는 viewport 또는 container 조건이 바뀔 때 크기와 disabled 상태를 동기화하고, `destroy()`는 pointer capture, observer, event listener, 임시 text-selection 상태와 controller가 바꾼 DOM 속성을 정리한다. Astro와 정적 HTML 소비처는 Svelte 컴포넌트 동작을 복제하지 않고 이 controller와 shared CSS를 쓴다.
...
Popover와 Menu는 설정, 더보기, 필터, 계정 메뉴처럼 가까운 트리거에서 짧게 펼치는 표면만 제공하며 항목 노출, 권한, 라우팅, 필터 의미, 계정 상태 판단은 소비 앱이 계속 소유한다. Popover는 `closeOnEscape`와 `closeOnOutside`로 dismissal을 명시적으로 보호할 수 있고, 보호 중에도 non-modal 표면답게 outside target focus를 막지 않는다. ARIA Menu는 키보드 탈출을 보장해야 하므로 Escape/outside dismissal opt-out을 제공하지 않는다. 두 컴포넌트는 light DOM과 open shadow root에서 composed click path를 기준으로 내부·외부 입력을 구분하고, trigger focus 복귀, 실제 click target focus 보존, menu keyboard movement 같은 기본 상호작용을 유지한다. closed shadow root는 내부 active element를 확인할 수 없으므로 이 focus 계약의 지원 범위가 아니다. tour, validation, modal decision, floating collision engine 역할로 확장하지 않는다. 모바일 keyboard, 긴 옵션, async option, grouped option, virtualized list, collision 반복 요구는 Popover/Menu/Combobox 안에서 계속 키우지 않고 Sheet flow 또는 headless spike로 보낸다.
...
Combobox는 검색 가능한 단일 선택의 label, input frame, listbox, active option, disabled option skip, hidden submitted value만 제공한다. 사용자가 선택 label을 편집하면 hidden submitted value를 즉시 비우고 `onValueChange('', null)`을 호출한다. `required`는 임의 입력 문자열이 아니라 실제 option 선택을 검증하며 기본 문구는 `selectionRequiredText`로 바꿀 수 있다. IME 조합 중 Enter와 방향키는 선택이나 이동으로 처리하지 않는다. 기본 input/listbox/option ID는 SSR과 hydration에서 안정적이며, 소비 앱이 명시적인 `id`를 전달하는 계약도 유지한다. light DOM과 stylesheet가 설치된 open shadow root에서 composed click path를 기준으로 자체 input/toggle/option 입력과 실제 outside click을 구분한다. 실제 필터링, async search, command 실행, 권한 판단은 소비 앱이 계속 소유한다. 단순 상태 선택은 native `Select`를 유지한다. `appearance: base-select`를 지원하는 브라우저에서는 picker surface, option 사이의 2 CSS px 간격과 각 option의 2 CSS px block inset, hover와 selected option, 그리고 inline-end에서 4 CSS px 떨어진 picker icon이 ZDP theme token과 spacing 계약을 따른다. 지원하지 않거나 forced-colors가 활성화된 환경에서는 native 동작과 사용자 색상 강제를 유지한다. 사용자가 입력으로 후보를 좁히는 단일 선택일 때만 Combobox를 쓴다.
...
TermTrigger와 TermSheet는 용어 설명을 클릭으로 여는 trigger, right sheet, bottom sheet 표면만 제공한다. `.zdp-term-trigger`는 본문 안 용어를 button으로 열고, sheet가 열린 동안에만 `aria-controls`를 연결하며, `.zdp-term-sheet`는 선택된 용어의 짧은 설명, 긴 설명, 예시, 관련 용어, detail link 자리를 보여준다. TermSheet의 기본 dialog/title/description ID는 SSR과 hydration에서 안정적이며, 소비 앱이 명시적인 `id`를 전달하는 계약도 유지한다. `.zdp-term-trigger`는 inline interactive text라서 hover에서 글자색을 바꾸지 않고 배경만 강조하며, 좌우 최소 padding과 focus token을 유지하고, 본문 텍스트 선택을 막지 않는다. Escape 닫기, backdrop 닫기, Tab 순환, scroll lock, 이전 focus 복귀는 sheet 표면의 기본 동작으로 유지한다. glossary manifest, locale fallback, 공개 가능 여부, detail page 라우팅 판단은 소비 앱과 glossary pipeline이 계속 소유한다.
