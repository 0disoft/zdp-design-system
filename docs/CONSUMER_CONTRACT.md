...
- standalone consumer, public template, external example처럼 sibling checkout을 전제로 하지 않는 표면은 npm registry package를 사용한다. 0.51.2 이상에서는 `zdp-design-system: ^0.51.2`를 기본 semver 범위로 쓰고, 재현 가능한 release proof가 필요한 곳만 exact version을 pin한다.
...
Dismissible surface는 닫혀 있을 때 component별 document listener를 유지하지 않는다. 열린 Menu, Popover, Combobox는 owner document의 공용 dismiss stack에 활성 순서로 등록되고, 문서당 공유 capture click/keydown listener가 최상단 layer 하나에만 outside click 또는 Escape를 전달한다. `closeOnEscape=false`나 `closeOnOutside=false`인 최상단 layer는 해당 입력이 아래 layer로 관통하지 않게 막는다. light DOM과 open shadow root는 `composedPath()` 기준으로 내부 입력과 outside 입력을 구분한다.
...
TermTrigger와 TermSheet는 용어 설명을 클릭으로 여는 trigger, right sheet, bottom sheet 표면만 제공한다. `.zdp-term-trigger`는 본문 안 용어를 button으로 열고, sheet가 열린 동안에만 `aria-controls`를 연결하며, `.zdp-term-sheet`는 선택된 용어의 짧은 설명, 긴 설명, 예시, 관련 용어, detail link 자리를 보여준다. TermSheet는 `open` 요청과 non-null `term`이 함께 있을 때만 modal lifecycle을 활성화하므로 비동기 term이 나중에 도착하거나 열린 상태에서 사라져도 DOM, scroll lock, background `inert`, focus 복귀가 같은 상태를 유지한다. 기본 dialog/title/description ID는 SSR과 hydration에서 안정적이며, 소비 앱이 명시적인 `id`를 전달하는 계약도 유지한다. `.zdp-term-trigger`는 inline interactive text라서 hover에서 글자색을 바꾸지 않고 배경만 강조하며, 좌우 최소 padding과 focus token을 유지하고, 본문 텍스트 선택을 막지 않는다. Escape 닫기, backdrop 닫기, Tab 순환, scroll lock, 이전 focus 복귀는 sheet 표면의 기본 동작으로 유지한다. glossary manifest, locale fallback, 공개 가능 여부, detail page 라우팅 판단은 소비 앱과 glossary pipeline이 계속 소유한다.
