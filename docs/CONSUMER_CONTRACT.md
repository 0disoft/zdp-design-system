...
PageHeader는 제목, 요약, 액션 배치만 제공하며 문구, SEO heading hierarchy, 라우팅, 데이터, 권한 판단은 소비 앱이 계속 소유한다.
...
- `control.heightMd`, `control.glyphMd`, `control.radius`, `control.borderWidth`, `control.hitTarget`은 native control size와 icon glyph size를 맞출 때의 기준이다.
