...
- standalone consumer, public template, external example처럼 sibling checkout을 전제로 하지 않는 표면은 npm registry package를 사용한다. 0.48.0 이상에서는 `zdp-design-system: ^0.48.0`를 기본 semver 범위로 쓰고, 재현 가능한 release proof가 필요한 곳만 exact version을 pin한다.
...
데스크톱 작업 영역은 `ResizableSplitPane` 또는 framework-neutral `createZdpSplitPaneController`, `.zdp-resizable-split-pane`, `.zdp-resizable-split-pane__separator`로 나누되 현재 폭과 저장 key, 모바일 전환점, Sheet 열림 상태는 소비처가 소유한다.
