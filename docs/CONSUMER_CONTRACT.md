...
- standalone consumer, public template, external example처럼 sibling checkout을 전제로 하지 않는 표면은 npm registry package를 사용한다. 0.47.1 이상에서는 `zdp-design-system: ^0.47.1`를 기본 semver 범위로 쓰고, 재현 가능한 release proof가 필요한 곳만 exact version을 pin한다.
...
Tabs는 가까운 정보 묶음 전환을 표현하되 라우팅, 권한, 데이터 로딩 판단은 소비 앱이 계속 소유한다. 기본 tab/panel prefix는 인스턴스마다 다르다. SSR/hydration 또는 테스트에서 결정적인 DOM id가 필요하면 소비 앱이 stable `idPrefix`를 넘긴다.
