...
Shared CSS is authored in the five ordered `src/styles/component-parts/*.css`
fragments. Run the configured `zdp_design_system_generate_component_styles`
intent after editing them. `src/styles/components.css` is generated; preview and
package builds reject stale output. Public CSS import paths and cascade order
remain unchanged, and the authoring fragments are not shipped in `dist`.

Svelte Tooltip wraps long text, flips when the opposite side fits, and shifts
within the visible viewport on open, resize and scrolling. It remains inside its
parent DOM: consumers must avoid clipping overflow ancestors and keep tooltip
...
- standalone consumer, public template, external example처럼 sibling checkout을 전제로 하지 않는 표면은 npm registry package를 사용한다. 0.61.1 이상에서는 `zdp-design-system: ^0.61.10`를 기본 semver 범위로 쓰고, 재현 가능한 release proof가 필요한 곳만 exact version을 pin한다.
...
- Rodisoft 회사 식별이 필요한 법인·명함·회사 소개 표면은 `rodi-mark.svg`를 정본으로 사용한다. `rodi-mark-1254.png`는 SVG를 처리하지 못하는 도구의 승인된 fallback이며 제품용 ship mark를 대체하지 않는다.
...
데스크톱 작업 영역은 `ResizableSplitPane` 또는 `zdp-design-system/split-pane`의 framework-neutral `createZdpSplitPaneController`, `.zdp-resizable-split-pane`, `.zdp-resizable-split-pane__separator`로 나누되 현재 폭과 저장 key, 모바일 전환점, Sheet 열림 상태는 소비처가 소유한다.
...
<Page labelledBy="consumer-contract-title">
  ...
  <Container as="section" id="content" size="lg" gutter="page">
    <Stack gap="md">
    <Breadcrumb
      ariaLabel="현재 위치"
      items={[
...
ConfirmAction은 중요한 액션을 밀기 또는 길게 누르기로 확인하는 표면이며 `onconfirm` 콜백만 전달한다. resting/hover/active/confirmed 상태는 outer border가 아니라 tonal surface, 진행 fill, thumb 색으로 구분하고 keyboard focus와 forced-colors에서만 의미 경계를 복원한다. 결제, 삭제, 권한, 환불 판단과 서버 요청은 소비 앱이 계속 소유한다.
...
ResizableSplitPane은 데스크톱의 primary/secondary 영역과 `role="separator"` 경계만 제공한다. `size`는 px 숫자로 bind하고 기본값 280, 기본 최소 220, 기본 최대 480을 사용한다. 글자 크기나 문장 길이로 primary 폭을 다시 계산하지 않으며, 긴 탐색 라벨의 말줄임과 내부 scroll은 소비 콘텐츠가 정한다. 경계선은 1px token으로 보이지만 기본 pointer hit area는 24px이고 Pointer Events capture, cancel/lost-capture cleanup, drag 중에만 적용되는 `.zdp-user-select-dragging`, Arrow/Home/End/PageUp/PageDown, Shift 가속, RTL 방향, double-click 기본 폭 복귀를 제공한다. 경계의 앞/뒤 절반을 한 번 클릭하면 한 step 이동하므로 drag가 어려운 pointer 사용자도 폭을 바꿀 수 있다.

`createZdpSplitPaneController({ root, primary, separator, secondary }, options)`는 같은 Pointer Events, keyboard, RTL, ResizeObserver clamp, class, CSS variable, `role="separator"`, `aria-orientation`, `aria-valuemin/max/now/text` 계약을 일반 DOM에 연결한다. 드래그 크기는 separator hit target 안에서 누른 위치와 무관하게 drag-start size에 실제 pointer delta만 더하므로 경계 양끝에서 잡아도 첫 이동에 크기가 튀지 않는다. drag-start에서 방향을 한 번 측정하고 pointer move는 animation frame당 한 번만 반영하며, pointer capture가 실패하거나 진행 중 orientation이 바뀌면 drag와 text-selection lock을 즉시 취소하고 혼합 축 값을 commit하지 않는다. Svelte binding이 controller의 내부 size를 그대로 되돌려 주는 update는 재측정·재렌더하지 않는다. Astro와 정적 HTML은 package root가 아니라 `zdp-design-system/split-pane`에서 import해 Svelte component barrel을 Vite dev dependency graph에 넣지 않는다. import 자체는 SSR-safe이며 DOM 조회와 controller 생성은 브라우저 lifecycle에서만 수행한다. `update()`는 viewport 또는 container 조건이 바뀔 때 크기와 disabled 상태를 동기화하고, `destroy()`는 pointer capture, pending animation frame, observer, event listener, 임시 text-selection 상태와 controller가 바꾼 DOM 속성을 정리한다. Astro와 정적 HTML 소비처는 Svelte 컴포넌트 동작을 복제하지 않고 이 controller와 shared CSS를 쓴다.

`createZdpSplitPaneSizePersistence({ key })`는 versioned localStorage key, 숫자 검증, 220–480 clamp, 저장소 예외 처리만 제공한다. helper 생성은 SSR-safe이고 `load`/`save` 호출은 소비 앱이 `onMount`와 `onResizeCommit`에서 명시적으로 연결한다. 컴포넌트 안에는 숨은 저장이나 전역 layout state가 없다. `secondaryMinSize`는 부모 컨테이너에서 남겨야 할 공간을 선언하며, 둘을 동시에 만족할 수 없는 폭에서는 `data-zdp-resizable-split-pane-constrained="true"`가 표시된다. 이 상태를 모바일 layout으로 버티게 하지 말고 소비 앱의 container/media 조건에서 split pane을 제거하고 `Sheet placement="left"` 또는 접힘 메뉴로 교체한다.
Tooltip은 짧은 보조 설명 표면만 제공하며 light DOM과 stylesheet가 설치된 open shadow root에서 Escape dismiss와 trigger focus를 유지하고, 긴 도움말, validation message, popover, tour, 권한 판단은 소비 앱이 계속 소유한다. 설명이 접근성 이름을 보강해야 할 때만 Tooltip `id`를 넘기고 trigger에 `aria-describedby`를 연결한다.
...
Popover와 Menu는 설정, 더보기, 필터, 계정 메뉴처럼 가까운 트리거에서 짧게 펼치는 표면만 제공하며 항목 노출, 권한, 라우팅, 필터 의미, 계정 상태 판단은 소비 앱이 계속 소유한다. Menu trigger/panel/item은 tonal surface로 계층을 구분하고 resting/hover border를 사용하지 않는다. Popover는 `closeOnEscape`와 `closeOnOutside`로 dismissal을 명시적으로 보호할 수 있고, 보호 중에도 non-modal 표면답게 outside target focus를 막지 않는다. ARIA Menu는 키보드 탈출을 보장해야 하므로 Escape/outside dismissal opt-out을 제공하지 않는다. 두 컴포넌트는 light DOM과 open shadow root에서 composed click path를 기준으로 내부·외부 입력을 구분하고, trigger focus 복귀, 실제 click target focus 보존, menu keyboard movement 같은 기본 상호작용을 유지한다. closed shadow root는 내부 active element를 확인할 수 없으므로 이 focus 계약의 지원 범위가 아니다. tour, validation, modal decision, floating collision engine 역할로 확장하지 않는다. 모바일 keyboard, 긴 옵션, async option, grouped option, virtualized list, collision 반복 요구는 Popover/Menu/Combobox 안에서 계속 키우지 않고 Sheet flow 또는 headless spike로 보낸다.

Disclosure, LocaleSwitcher, SegmentedControl, TextScaleControl, Menu, Popover, Tooltip은 명시적 `id` 또는 `idPrefix`가 없을 때도 반복 SSR과 hydration 사이에서 같은 fallback ID와 ARIA 참조를 유지한다. 외부 DOM과 참조를 공유하거나 고정 selector가 필요한 소비처만 명시적 값을 제공한다.
Sheet는 right, left, bottom edge panel로 설정, 필터, 보조 흐름을 여는 modal surface만 제공한다. Escape 닫기, backdrop 닫기, Tab 순환, scroll lock, background `inert`, 이전 focus 복귀, `role="dialog"`, `aria-modal` 구조는 Sheet 표면의 기본 동작으로 유지한다. `closeOnEscape` 또는 `closeOnBackdrop`을 끄면 해당 입력은 modal을 닫지 않는다. 비활성 backdrop은 button이나 Close action으로 노출하지 않고 접근성 트리에서 숨기며, pointer 입력 뒤에도 현재 panel focus를 유지한다. Dialog, Sheet, TermSheet가 겹치면 컴포넌트 종류별 고정 순서가 아니라 열린 순서가 실제 stacking order를 결정하고, 아래 layer가 먼저 닫혀도 위 layer의 focus, background isolation, scroll lock을 유지한다. modal manager는 활성 top layer 바깥 DOM 가지를 임시로 inert 처리하고 final close에서 원래 inert 상태를 복구한다. 소비 앱의 조건부 렌더링이 활성 top component를 정상 close 이전에 제거해도 surviving trigger focus, inert, scroll lock, layer metadata를 복구한다. body-level portal host와 open shadow root 안에 modal component가 배치돼도 같은 isolation과 focus 계약을 유지하지만, design system이 portal target API나 mount orchestration을 제공하는 것은 아니다. closed shadow root는 내부 active element를 외부 component가 확인할 수 없으므로 이 focus 계약의 지원 범위가 아니다. Drawer는 별도 컴포넌트로 복제하지 않고 Sheet placement/use case로 먼저 다룬다. 저장, 권한, 데이터 fetch, 라우팅 판단은 소비 앱이 계속 소유한다.
...
Container는 페이지 폭, inline padding, centered width만 제공하며 그리드 의미, 라우팅, 데이터, 권한, 제품별 visibility 판단은 소비 앱이 계속 소유한다. 페이지 최외곽 shell은 `gutter="page"` 또는 정적 `.zdp-container--padding-page`를 사용해 max-width의 auto margin이 사라지는 폭에서도 양쪽 최소 24 CSS px를 유지한다. `padding="sm"`, `padding="md"`, `padding="lg"`는 중첩 Container용으로 계속 허용한다. 페이지 shell이 더 작은 gutter를 써야 한다면 의미 class를 조용히 빼는 대신 소비 저장소의 architecture 예외와 근거를 명시해야 한다. 이 semantic class/API는 architecture linter가 page shell의 작은 gutter를 식별할 수 있는 기준이다.
...
TermTrigger와 TermSheet는 용어 설명을 클릭으로 여는 trigger, right sheet, bottom sheet 표면만 제공한다. `.zdp-term-trigger`는 본문 안 용어를 button으로 열고, sheet가 열린 동안에만 `aria-controls`를 연결하며, `.zdp-term-sheet`는 선택된 용어의 짧은 설명, 긴 설명, 예시, 관련 용어만 보여준다. TermSheet는 detail page 링크나 라우팅 action을 제공하지 않는다. TermSheet는 `open` 요청과 non-null `term`이 함께 있을 때만 modal lifecycle을 활성화하므로 비동기 term이 나중에 도착하거나 열린 상태에서 사라져도 DOM, scroll lock, background `inert`, focus 복귀가 같은 상태를 유지한다. 기본 dialog/title/description ID는 SSR과 hydration에서 안정적이며, 소비 앱이 명시적인 `id`를 전달하는 계약도 유지한다. `.zdp-term-trigger`는 inline interactive text라서 기본 상태에 밑줄이나 입력 필드처럼 보이는 하단 경계를 두지 않고 옅은 강조 배경과 중간 굵기로 클릭 가능한 용어임을 구분한다. hover에서는 배경 형상을 바꾸거나 글자 굵기로 레이아웃을 흔들지 않고 strong ink 색으로 글자를 또렷하게 하며, 좌우 최소 padding과 focus token을 유지하고, 본문 텍스트 선택을 막지 않는다. Escape 닫기, backdrop 닫기, Tab 순환, scroll lock, 이전 focus 복귀는 sheet 표면의 기본 동작으로 유지한다. glossary manifest, locale fallback, 공개 가능 여부, detail page 라우팅 판단은 소비 앱과 glossary pipeline이 계속 소유한다.
...
Input, Textarea, Select는 `describedBy`에 하나 이상의 id를 연결할 수 있고 invalid 상태에서는 `errorMessageId`로 `aria-errormessage`를 연결한다. Select와 customizable native picker는 tonal surface와 투명 border slot을 사용하고, focus outline과 forced-colors system border만 의미 경계로 남긴다. Select invalid 상태는 별도 frame을 중첩하지 않고 `aria-invalid`, `aria-errormessage`, 연결된 ErrorText로 전달한다. 에러 메시지 문구, 검증 조건, 제출 차단 여부는 소비 앱이 결정한다.
Switch도 `describedBy` id 배열과 `errorMessageId`를 통해 invalid 상태의 보조 설명과 에러 메시지를 native switch input에 연결한다.
Input과 Textarea의 `readonly` 상태는 제출과 포커스를 유지하는 읽기 전용 값에 사용한다. 권한 때문에 값을 바꾸면 안 되는지, 단순히 고정 식별자를 보여주는지는 소비 앱이 결정한다. Input은 문자열 `value`가 소유하는 text·search·email·password·url·tel·number·date·time·datetime-local·month·week만 허용한다. checked 상태는 Checkbox·Radio, FileList는 native file input, hidden form transport는 native hidden input이 각각 소유한다. CommandField의 type은 search 또는 text만 허용한다.
...
- ResizableSplitPane 또는 DOM controller를 쓰는 화면은 px `size`와 고유 persistence key를 소비 앱이 소유하고, separator의 이름·현재값·Arrow/Home/End 동작, 24px 이상 hit area, pointer cancel cleanup, RTL, 200% zoom, 긴 말줄임을 확인한다. `920px` 이하나 좁은 부모에서는 split pane을 계속 찌그러뜨리지 말고 controller를 비활성화한 뒤 Sheet나 접힘 메뉴로 바꾼다. 본문 옆 공유 dock은 secondary 폭 안에 넣지 않고 바깥 `auto` track에 둔다.
- light/dark, keyboard focus, disabled, invalid 상태를 소비처 화면에서 확인한다.
- Theme / Locale Stress story에서 선택형 `locale-fonts.css`를 로드하고 light/dark, ZDP Mobile 폭, 12개 target locale(`en`, `zh`, `es`, `fr`, `hi`, `ko`, `ja`, `vi`, `ru`, `id`, `ms`, `th`)의 긴 문장과 focus-visible 상태가 함께 무너지지 않는지 확인한다. 초기 active locale이 `ko`, `en`이어도 QA fixture는 target locale 전체를 줄이지 않는다.
