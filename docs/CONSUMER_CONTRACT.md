...
- standalone consumer, public template, external example처럼 sibling checkout을 전제로 하지 않는 표면은 npm registry package를 사용한다. 0.57.2 이상에서는 `zdp-design-system: ^0.57.2`을 기본 semver 범위로 쓰고, 재현 가능한 release proof가 필요한 곳만 exact version을 pin한다.
...
- Theme / Locale Stress story에서 light/dark, ZDP Mobile 폭, 12개 target locale(`en`, `zh`, `es`, `fr`, `hi`, `ko`, `ja`, `vi`, `ru`, `id`, `ms`, `th`)의 긴 문장과 focus-visible 상태가 함께 무너지지 않는지 확인한다. 초기 active locale이 `ko`, `en`이어도 QA fixture는 target locale 전체를 줄이지 않는다.
